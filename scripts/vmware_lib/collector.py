...
def fetch_metadata(url: str = METADATA_URL, timeout: int = 30) -> dict:
    ...
    # 白名单校验防未来 URL 变量化时误开 file:// 等危险 scheme
    # 用显式 if/raise 而非 assert：assert 在 -O 优化模式下被忽略，会绕过安全检查
    if not url.startswith(("https://archive.org/", "http://archive.org/")):
        raise ValueError(f"unexpected URL scheme: {url}")
